...
├── run_tests.js         # 在終端機跑 tests/run.js（需要 jsdom）
└── make_icons.py
```
...
**沒有瀏覽器可用的時候**（CI，或就是開不起來）走這一支備援：

```bash
npm install jsdom && node tools/run_tests.js
```

它載入的是**同一份** `extension/src/*.js` 與**同一份** `tests/run.js`，
不是另外寫一套 Node 版測試——另寫一套等於測到別的東西。
離開碼：`0` 全過、`1` 有測試沒過、`2` 根本沒跑起來。
`2` 這一級刻意分開：載入失敗跟「零個測試通過」看起來都很像成功，
而這整個專案抓到的問題十之八九就是「安靜地什麼都沒做」。
...
- **訂閱牆偵測與 EZproxy 還原沒有在真站台實測。** 單元測試都過了，但我手上
  沒有機構訂閱帳號，沒辦法在真的付費牆或 proxy 網址上驗證。
  第一次遇到時請看一下警告有沒有正確出現。
